  ...
  io.on("connection", (socket) => {
    ...
    socket.on("startGame", () => {
      ...
      players.forEach((player, index) => {
        ...
        const twoOfClubsIdx = hand.findIndex(
          (obj) => obj.suit === "♣" && obj.value == 2
        );
        if (twoOfClubsIdx !== -1) {
          firstPlayer = player;
    ...
    socket.on("playCard", (card) => {
      ...
      if (!rD[rId].started) {
        ...
        if (rD[rId].suit == "" && !rD[rId].heartbreak && card.suit == "♥") {
          ...
          for (let i = 0; i < 13; i++) {
            if (
              rD[rId].hands[playerIdx][i] &&
              rD[rId].hands[playerIdx][i].suit != "♥"
            ) {
        ...
        if (rD[rId].suit != "" && card.suit != rD[rId].suit) {
          // established suit + not matching
          for (let i = 0; i < 13; i++) {
            // check for has suit
            if (
              rD[rId].hands[playerIdx][i] &&
              rD[rId].hands[playerIdx][i].suit == rD[rId].suit
            ) {
              // console.log("Play the suit if you have it.");
              io.to(socket.id).emit(
                "playerMsg",
                "Play the suit if you have it."
              );
              return;
            }
          }
      ...
      const newHand = roomUtils.playCard(rD[rId].hands[playerIdx], card);
      // update the data with the new hand
      rD[rId].hands[playerIdx] = newHand;
      // differences: information changed, actions
      if (rD[rId].center[0] == null) {
        // CASE 1: first card played
        // console.log(rD[rId])
      ...
      } else if (rD[rId].center[2] == null) {
        // CASE 2: middle card (3rd card hasn't been played)
        rD[rId].turn = (rD[rId].turn + 1) % 4;
        ...
        if (
          possibleHighest &&
          CARD_VALUE_MAP[card.value] > rD[rId].highestValue
        ) {
          rD[rId].highestValue = CARD_VALUE_MAP[card.value];
          rD[rId].highestId = socket.id;
        }
        ...
        for (let i = 0; i < 4; i++) {
          ...
          if (
            rD[rId].center[i].suit == "♠" &&
            rD[rId].center[i].value == "Q"
          ) {
            roundPts += 13;
          }
        }
        ...
        const winnerIdx = players.indexOf(rD[rId].highestId);
        ...
        if (rD[rId].hands[0].every((element) => element === null)) {
          ...
          setTimeout(() => {
            ...
            if (Math.max(...rD[rId].totalPts) >= 25) {
              // 10/25 right now for testing (1-2 rounds), normally 75/100
        ...
        setTimeout(() => {
          ...
          if (rD[rId].heartbreak) {
            // hearts broken, all non-empty slots are valid
            ...
            const hasOnlyHeartsIdx = playerHand.findIndex(
              (obj) =>
                obj &&
                (obj.suit == "♦" || obj.suit == "♣" || obj.suit == "♠")
            );
            if (hasOnlyHeartsIdx == -1) {
              // only hearts, all non-empty are valid
              playerHand.forEach((card, index) => {
                if (card !== null) {
                  validCards[index] = true;
                }
              ...
              playerHand.forEach((card, index) => {
                // hearts not broken, can't play hearts
      ...
      // update after turn (when not the last card)
      io.to(rId).emit("updateCenter", rD[rId].center);
      io.to(socket.id).emit("updateHand", newHand);
      io.to(rId).emit("serverMsg", `Player ${rD[rId].turn + 1}'s Turn!`);
      // if have suit, must play. else, all non-empty slots are valid
      validCards = new Array(13).fill(false);
      const hasSuitIdx = rD[rId].hands[rD[rId].turn].findIndex(
        (obj) => obj && obj.suit == rD[rId].suit
      );
      const playerHand = rD[rId].hands[rD[rId].turn];
      if (hasSuitIdx == -1) {
        // suit not present, all non-empty slots are valid
        playerHand.forEach((card, index) => {
          if (card !== null) {
            validCards[index] = true;
          }
